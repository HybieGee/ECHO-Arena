// Script to create 50 test bots with diverse strategies
...
const walletAddress = "0x3587e9fe0a1e0ba8a3c6bcfb0d7d5870c08b2c1d";
const matchId = 3;

// Generate SQL insert statements
console.log("-- Insert 50 test bots for match simulation");
console.log("-- Run this in wrangler d1 execute\n");

botPrompts.forEach((prompt, index) => {
  const botName = `Test Bot ${index + 1}`;
  const sql = `INSERT INTO bots (owner_address, match_id, prompt, prompt_dsl, bot_name) VALUES ('${walletAddress}', ${matchId}, '${prompt.replace(/'/g, "''")}', '{}', '${botName}');`;
  console.log(sql);
});
